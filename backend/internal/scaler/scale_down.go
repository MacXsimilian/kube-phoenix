...
func (r *Runner) drainAndDeleteNode(ctx context.Context, mode, name string, podCount int, drainTimeout time.Duration, logCh chan<- LogLine, counts *Counts) {
	if !isApply(mode) {
		emit(logCh, "plan", fmt.Sprintf("Would drain node %s (pods=%d timeout=%s)", name, podCount, drainTimeout))
		emit(logCh, "plan", fmt.Sprintf("Would delete node object %s", name))
		counts.Drained++
		counts.Deleted++
		return
	}

	emit(logCh, "info", fmt.Sprintf("Draining node %s (pods=%d timeout=%s)...", name, podCount, drainTimeout))
	if err := r.k8s.DrainNode(ctx, name, drainTimeout); err != nil {
		emit(logCh, "error", fmt.Sprintf("Drain failed for %s: %s", name, err))
		counts.Errors++
		return
	}
	emit(logCh, "ok", fmt.Sprintf("Drained node %s", name))
	counts.Drained++

	if err := r.k8s.DeleteNode(ctx, name); err != nil {
		emit(logCh, "error", fmt.Sprintf("Failed to delete node %s: %s", name, err))
		counts.Errors++
	} else {
		emit(logCh, "ok", fmt.Sprintf("Deleted node object %s", name))
		counts.Deleted++
	}
}
